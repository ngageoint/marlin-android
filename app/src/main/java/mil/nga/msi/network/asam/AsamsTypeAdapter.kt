...
class AsamsTypeAdapter: TypeAdapter<List<Asam>>() {
   ...
   private fun readAsam(`in`: JsonReader): Asam? {
      ...
      return if (reference != null && date != null && latitude != null && longitude != null) {
        val asam = Asam(reference, date, latitude, longitude)
         asam.position = position
         asam.navigationArea = navigationArea
         asam.subregion = subregion
         asam.description = description
