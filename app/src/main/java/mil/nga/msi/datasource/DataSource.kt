package mil.nga.msi.datasource

import androidx.compose.ui.graphics.Color
import mil.nga.msi.R

enum class DataSource(
   val mappable: Boolean,
   val color: Color,
   val icon: Int,
   // TODO: move to ui map package
   val imageScale: Float = .75f,
   val label: String,
   val labelPlural: String = "${label}s"
) {
   ASAM(true, Color(0xFF000000), R.drawable.ic_asam_24dp, label = "ASAM"),
   MODU(true, Color(0xFF0042A4), R.drawable.ic_modu_24dp, label = "MODU"),
   NAVIGATION_WARNING(false, Color(0xFFD32F2F), R.drawable.ic_round_warning_24, label = "Navigational Warning"),
   LIGHT(true, Color(0xFFFFC500), R.drawable.ic_baseline_lightbulb_24, label = "Light"),
   PORT(true, Color(0xFF5856D6), R.drawable.ic_baseline_anchor_24, label = "World Port"),
   RADIO_BEACON(true, Color(0xFF007BFF), R.drawable.ic_baseline_settings_input_antenna_24, label = "Radio Beacon"),
   DGPS_STATION(true, Color(0xFFFFB300), R.drawable.ic_dgps_icon_24, label = "Differential GPS Station"),
   ELECTRONIC_PUBLICATION(false, Color(0xFF30B0C7), R.drawable.ic_description_24dp, label = "Electronic Publication");

   fun labelForCount(count: Int) = if (count == 1) label else labelPlural
}