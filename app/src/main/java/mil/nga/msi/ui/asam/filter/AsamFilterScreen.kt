...
private fun AsamFilters(
   filters: List<AsamFilter>,
   removeFilter: (AsamFilter) -> Unit
) {
...
private fun valueString(
   ...
   return when (parameter.type) {
      ParameterType.DATE -> { dateValue(value = value) }
      ParameterType.DOUBLE -> { doubleValue(value) }
      ParameterType.LOCATION -> { locationValue(value) }
      ParameterType.STRING -> { stringValue(value ) }
      else -> { "" }
   }
}
...
private fun AsamFilterHeader(
   addFilter: (AsamFilter) -> Unit
) {
   val defaultParameter = AsamParameters().parameters.first()
      ...
      Column(
         ...
         Row(
            ...
            ParameterSelection(
               ...
               onSelectParameter = {
                  parameter = it
                  value = if (parameter.type == ParameterType.DATE) "last 30 days" else ""
            ...
            if (parameter.type != ParameterType.STRING && parameter.type != ParameterType.LOCATION) {
               Row(
                  Modifier
                     .weight(1f)
...
private fun ParameterSelection(
   selectedParameter: AsamParameter,
   onSelectParameter: (AsamParameter) -> Unit,
   modifier: Modifier = Modifier,
) {
   var expanded by remember { mutableStateOf(false) }

   val parameters = AsamParameters()
